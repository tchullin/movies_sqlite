...
class Functions():
    ...
    def add_category(self):
        ...
        self.db_conect()
        self.cursor.execute("""INSERT INTO categorias (name) VALUES(?)""",(self.name,))
    ...
    def select_list(self):
        ...
        self.db_conect()
        list = self.cursor.execute("""SELECT id , name FROM categories ORDER BY name ASC;""")
        count = 0
        for l in list:
            if count % 2 == 0:
    ...
    def delete_categoria(self):
        self.capture_entries()
        self.db_conect()
        self.cursor.execute("""DELETE FROM categorias WHERE id = ?""",(self.id,))
        self.connection.commit()
        self.db_desconect()
        self.Clear_data()
        self.select_list()
    ...
    def update_categoria(self):
        self.capture_entries()
        self.db_conect()
        self.cursor.execute("""UPDATE categorias SET name = ? WHERE id = ?;""",(self.name, self.id))
        self.connection.commit()
        self.db_desconect()
        self.Clear_data()
        self.select_list()
    ...
    def Search_categoria(self):
        ...
        self.cursor.execute("""SELECT * FROM categorias WHERE name LIKE '%s' COLLATE NOCASE ORDER BY name ASC""" % name)
        result_search = self.cursor.fetchall()
        for categoria in result_search:
            self.list_grid.insert("",END,values=categoria)
        self.db_desconect()
        self.Clear_data()
        self.db_desconect()
...
class Aplication(Functions):
    ...
    def frames_screen(self):
        self.frame1 = LabelFrame(self.root, bd=2, text="Category Table Maintenance")
        self.frame1.place(relx=0.02, rely=0.02, relwidth=0.96, relheight=0.46)
        self.frame2 = LabelFrame(self.root, bd=2, text="Category Table List")
    ...
    def widgets_frame1(self):
        ...
        # botão Search
        self.bt_Search = Button(self.frame1, text="Search", command=self.Search_categoria)
        self.bt_Search.place(relx=0.3, rely=0.1, relwidth=0.1, relheight=0.15)

        # botão Add
        self.bt_Add = Button(self.frame1, text="Add", command=self.add_category)
        self.bt_Add.place(relx=0.6, rely=0.1, relwidth=0.1, relheight=0.15)

        # Botão Update
        self.bt_update = Button(self.frame1, text="update", command=self.update_categoria)
        self.bt_update.place(relx=0.7, rely=0.1, relwidth=0.1, relheight=0.15)

        # Botão Delete
        self.bt_Delete = Button(self.frame1, text="Delete", command=self.delete_categoria)
